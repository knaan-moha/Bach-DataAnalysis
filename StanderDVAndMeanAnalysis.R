  ...
  for (x in 1:num_columns) {
    if ((x - 1) %% 5 == 0) {
      if ((x - 1) %% 15 == 0) {
        print(" ")
        print(" ")
        print(sprintf("%s ", colnames(react_data_frame)[x]))
      }
    ...
    if (any(is.na(react_values)) || any(is.na(blazor_values))) {
      print("Data contains non-numeric values or NAs, check the dataset")
    } else {
      ...
      print(sprintf("React %s: %f; Blazor: %f", colnames(react_data_frame)[x], react_sd, blazor_sd))
      print("Means")
      print(sprintf("React %s: %f; Blazor: %f", colnames(react_data_frame)[x], react_mean, blazor_mean))
    }
  }
}

# Assuming react_performance and blazor_performance are variables containing the data frames
# If not, replace them with react_data_frame and blazor_data_frame
