check_standard_deviation <- function(react_data_frame, blazor_data_frame) {
  dimensions <- dim(react_data_frame)
  num_columns <- dimensions[2]
  
  print("BLAZOR")
  
  for (x in 1:num_columns) {
    # Trigger new header and section every 15 columns
    if ((x - 1) %% 15 == 0) {
      print(" ")
      print(" ")
      header_info <- react_data_frame[1, x]
      print(header_info)
    }
    
    if ((x - 1) %% 5 == 0) {
      metric_name <- react_data_frame[2, x]
      print(sprintf("===================  %s  =======================", metric_name))
      print(sprintf("===================  %s =======================", react_data_frame[1, x]))
    }
    
    # Convert the data range to numeric
    react_values <- as.numeric(unlist(react_data_frame[3:22, x]))
    blazor_values <- as.numeric(unlist(blazor_data_frame[3:22, x]))
    
    # Check if data conversion was successful (no NAs)
    if (!any(is.na(react_values)) && !any(is.na(blazor_values))) {
      react_sd <- sd(react_values, na.rm = TRUE)
      blazor_sd <- sd(blazor_values, na.rm = TRUE)
      react_mean <- mean(react_values, na.rm = TRUE)
      blazor_mean <- mean(blazor_values, na.rm = TRUE)
      
      print("Standard Deviation")
      print(sprintf("React %s: %.3f; Blazor: %.3f", metric_name, react_sd, blazor_sd))
      print("Means")
      print(sprintf("React %s: %.3f; Blazor: %.3f", metric_name, react_mean, blazor_mean))
    } else {
      print(sprintf("Data for column %s contains non-numeric values or NAs, skipping calculations.", metric_name))
    }
  }
}

check_standard_deviation(react_data_frame, blazor_data_frame)